...
const   DateRangePicker = (props) => {
    ...
    const nextWeek = moment().locale("en").startOf('week').format(dateFormat.NATIVE);
    const lastWeek = moment().locale("en").subtract(1, 'week').startOf('isoWeek').format(dateFormat.NATIVE);
    const startMonth = moment().locale("en").startOf('month').format(dateFormat.NATIVE);
    const nextMonth = moment().locale("en").endOf('month').format(dateFormat.NATIVE);
    const startOfLastMonth = moment().locale("en").subtract(1, 'months').startOf('month').format(dateFormat.NATIVE);
    const endOfLastMonth = moment().locale("en").subtract(1, 'months').endOf('month').format(dateFormat.NATIVE);
    const interStartMediateDate = moment(selectedMinDate).locale("en").format(dateFormat.NATIVE);
    const interMediateEndDate = moment(selectedMaxDate).locale("en").format(dateFormat.NATIVE);
    const inputToady = moment().locale("en").format(dateFormat.CHART_DATE);
    const inputNextWeek = moment().locale("en").startOf('week').format(dateFormat.CHART_DATE);
    const inputLastWeek = moment().locale("en").subtract(1, 'week').startOf('isoWeek').format(dateFormat.CHART_DATE);
    const inputStartMonth = moment().locale("en").startOf('month').format(dateFormat.CHART_DATE);
    const inputNextMonth = moment().locale("en").endOf('month').format(dateFormat.CHART_DATE);
    const inputStartOfLastMonth = moment().locale("en").subtract(1, 'months').startOf('month').format(dateFormat.CHART_DATE);
    const inputEndOfLastMonth = moment().locale("en").subtract(1, 'months').endOf('month').format(dateFormat.CHART_DATE);
    ...
    const onSelectDate = (type) => {
        switch (type) {
            case dateLabelSelector.CLEAN:
                ...
                setDateColor('clear');
                setPopoverOpen(false);
                setSelectedMinDate(moment().locale("en").startOf('month').toDate());
                setSelectedMaxDate(moment().locale("en").endOf('month').toDate())
    ...
    const onDatepickerRef = (el) => {
        if (el && el.input) {
            ...
            el.input.style.cursor = 'pointer';
        }
    }

    // ✅ Configuración correcta de transición para reactstrap
            ...
            <div id='Popover1'>
                <Form.Group className='position-relative' controlId='formBasicDate'>
                    <Form.Control
                        ...
                        placeholder={placeholderText('date-picker.filter.placeholder.label')}
                        readOnly={true}
            ...
            <div className={`${popoverOpen === true ? 'custom-overlay' : ''}`}>
                ...
                >
                    <PopoverBody className='date-picker-popover'>
                        <ListGroup>
                            <ListGroupItem
                                className={`${dateColor === 'today' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                            ...
                            <ListGroupItem
                                className={`${dateColor === 'this_week' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                            ...
                            <ListGroupItem
                                className={`${dateColor === 'last_week' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                            ...
                            <ListGroupItem
                                className={`${dateColor === 'this_month' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                            ...
                            <ListGroupItem
                                className={`${dateColor === 'last_month' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                            ...
                            <ListGroupItem
                                className={`${dateColor === 'custom' ? 'bg-primary text-white' : ''} border-0 rounded cursor-pointer`}
                                style={{ cursor: 'pointer' }}
                            >
                                ...
                                <Popover
                                    trigger='legacy'
                                    placement='left'
                                    className='date-picker__child-popover'
                                            ...
                                            <Col md={6} xs={12}>
                                                <div className='datepicker mb-4 mb-lg-0 p-0'>
                                                    <DatePicker
                                                        ...
                                                        className='datepicker__custom-datepicker px-3'
                                                        name='date'
                                                        selected={selectedMinDate}
                                                        dateFormat='yyyy/MM/dd'
                                                        startOpen={false}
                                                        onChange={(date) => onSelectMinDate(date)}
                                            ...
                                            <Col md={6} xs={12}>
                                                <div className='datepicker p-0'>
                                                    <DatePicker
                                                        ...
                                                        dateFormat='yyyy/MM/dd'
                                                        startOpen={false}
                                                        onChange={(date) => onSelectMaxDate(date)}
                            ...
                            <div className='mt-3 text-center d-flex flex-row pl-4 align-items-center'>
                                ...
                                <button
                                    className={`btn btn-secondary ${applyButton ? 'ml-3' : ''}`}
...
DateRangePicker.propTypes = {
    ...
    onDateSelector: PropTypes.func.isRequired,
    isProfitReport: PropTypes.bool,
};
